...
SAVE_RESULT = True
# YANGI: OUTPUT_FILENAME endi ishlatilmaydi -- video fayli avtomatik ravishda
# "trains" papkasi ichiga, HAR SAFAR BOSHQA (vaqt tamg'ali) nom bilan
# saqlanadi, shuning uchun oldingi saqlangan video hech qachon ustidan
# yozilib ketmaydi/o'chmaydi.
OUTPUT_FILENAME = 'realtime_result.mp4'  # eski o'zgaruvchi -- endi shablon sifatida ishlatiladi
SAVE_RESIZE_WIDTH = 960  # YANGI: agar sonli qiymat (masalan 1280) berilsa, saqlanadigan
                            # video shu kenglikka kichraytiriladi -- fayl hajmi sezilarli
                            # kamayadi. None bo'lsa, asl o'lcham saqlanadi.
...
# FINAL_OUTPUT_PATH endi run_realtime() ichida, har safar YANGI (vaqt
# tamg'ali) nom bilan, "trains" papkasi ichida hisoblanadi (pastga qarang).
...
SHOW_RECOGNIZED_LIST = True   # YANGI: aniqlangan vagon raqamlari ro'yxatini
                                # yuqori o'ng burchakda ko'rsatish
...
RECOGNIZED_LIST_MARGIN_BOTTOM = 20  # pastda qancha joy bo'sh qoldirilsin
MIN_LIST_FONT_SCALE = 0.45   # YANGI: shrift bundan kichik bo'lmaydi (kichik videolarda ham o'qish mumkin bo'lsin)
MIN_LIST_LINE_HEIGHT = 16    # YANGI: qatorlar orasi bundan kichik bo'lmaydi
DISPLAY_MIN_WIDTH = 1280     # YANGI: ekranda ko'rsatiladigan oyna shu kenglikdan kichik bo'lsa, kattalashtiriladi
...
TRAIN_SESSION_GAP_SECONDS = 300   # 45 -> 300 ga oshirildi: video faylni qayta ishlash
                                    # kompyuter tezligiga qarab sekinlashishi mumkin, va bu
                                    # ikkita vagon orasidagi HAQIQIY (devor) vaqtini oshirib
                                    # yuboradi -- natijada bitta poyezd soxta ravishda bir
                                    # necha faylga bo'linib ketardi. Live kamerada haqiqiy
                                    # poyezdlar orasida odatda 5 daqiqadan ko'proq tanaffus
                                    # bo'ladi, shuning uchun bu qiymat live uchun ham xavfsiz.
...
class TrainSessionManager:
    ...
    Har bir "poyezd o'tishi" (session) ni boshqaradi:
      ...
      - agar uzoq vaqt (TRAIN_SESSION_GAP_SECONDS) hech qanday vagon o'qilmasa
        -- session yopiladi va natija faylga YOZIB QO'SHILADI (append)

    YANGILANDI: endi har safar YANGI fayl yaratish o'rniga, BITTA kunlik
    umumiy faylga ("<sana>_barcha_natijalar.txt") natijalar QO'SHIB
    (append) boriladi. Ya'ni eski yozuvlar hech qachon o'chirilmaydi yoki
    ustidan yozilmaydi -- har bir yangi video/poyezd natijasi shu faylning
    ENG PASTIGA qo'shilib ketaveradi.
    """
    ...
    def add_wagon(self, number_str, is_valid):
    ...
    def close(self):
        ...
        # YANGI: bitta kunlik umumiy fayl -- har bir video/poyezd shu faylga
        # QO'SHIB (append) yoziladi, eskisi hech qachon o'chirilmaydi/almashtirilmaydi.
        date_str = start_dt.strftime('%Y.%m.%d')
        filename = f"{date_str}_barcha_natijalar.txt"
        filepath = os.path.join(TRAINS_OUTPUT_DIR, filename)

        # MUHIM: Windows fayl nomida ":" ishlatib bo'lmaydi, shuning uchun
        # soat:daqiqa orasida "." ishlatildi (masalan 01.24, 01:24 emas)
        start_time_str = start_dt.strftime('%H.%M.%S')
        end_time_str = end_dt.strftime('%H.%M.%S')

        valid_count = sum(1 for _, _, v in self.wagons if v)

        # YANGI: shu kunlik faylda nechta poyezd yozuvi borligini hisoblab,
        # yangisiga tartib raqami ("1-poyezd", "2-poyezd", ...) beramiz
        train_number = 1
        if os.path.exists(filepath):
            existing_content = open(filepath, encoding='utf-8').read()
            train_number = existing_content.count("-POYEZD") + 1

        # MUHIM O'ZGARISH: mode="w" (ustidan yozish) o'rniga mode="a" (oxiriga
        # qo'shib yozish) ishlatilmoqda -- shu tufayli eski yozuvlar saqlanib
        # qoladi, yangisi esa faylning ENG PASTIGA qo'shiladi.
...
def process_finished_track(track_id, scores_dict, lifespan, stats, any_digit_seen):
...
def run_realtime():
    ...
    # YANGI: UI elementlari (raqamlar ro'yxati) endi video O'LCHAMIGA
    # nisbatan avtomatik moslashadi. RECOGNIZED_LIST_* konstantalari 720p
    # video uchun mo'ljallangan "asl" qiymatlar edi -- boshqa o'lchamdagi
    # videoda ular shu nisbatga (ui_scale) ko'paytiriladi, shunda Full HD
    # yoki undan katta videoda ham matn juda kichik bo'lib qolmaydi.
    ...
    # chiqish video o'lchamini hisoblaymiz (agar kichraytirish so'ralgan bo'lsa)
    ...
    out = None
    if SAVE_RESULT:
        # YANGI: video fayli "trains" papkasi ichiga, HAR SAFAR BOSHQA
        # (vaqt tamg'ali) nom bilan saqlanadi -- shuning uchun oldingi
        # saqlangan video hech qachon ustidan yozilib ketmaydi.
        os.makedirs(TRAINS_OUTPUT_DIR, exist_ok=True)
        base_name, ext = os.path.splitext(OUTPUT_FILENAME)
        timestamp_str = datetime.datetime.now().strftime('%Y.%m.%d_%H.%M.%S')
        FINAL_OUTPUT_PATH = os.path.join(TRAINS_OUTPUT_DIR, f"{timestamp_str}_{base_name}{ext}")

        # avval H.264 (avc1) kodekni sinaymiz -- bu mp4v'ga qaraganda
        # bir necha barobar kichikroq fayl beradi. Agar tizimda mavjud bo'lmasa
        # (Windows'da ba'zan shunday bo'ladi), eskisi (mp4v)ga qaytamiz.
        fourcc_h264 = cv2.VideoWriter_fourcc(*'avc1')
        out = cv2.VideoWriter(FINAL_OUTPUT_PATH, fourcc_h264, fps, (save_width, save_height))
        if not out.isOpened():
            print("⚠️  H.264 (avc1) kodek topilmadi, mp4v kodekka qaytilmoqda (fayl kattaroq bo'ladi)")
            out = cv2.VideoWriter(FINAL_OUTPUT_PATH, cv2.VideoWriter_fourcc(*'mp4v'), fps, (save_width, save_height))
        else:
            print("✅ H.264 (avc1) kodek bilan saqlanmoqda (siqilgan)")
    ...
    track_any_digit_seen = defaultdict(bool)  # shu track umrida hech bo'lmasa bitta
    ...
    while True:
                        ...
                        if detected_digits:
                            ...
                            if len(final_number_str) == 8 and avg_conf > 0.5:
                                ...
                                if SHOW_LIVE_LABEL:
                                    ...
                                    # Chiroyli, o'rtacha o'lchamdagi ramkali yozuv
                                    font_scale = 1.3
                                    thickness = 3
                                    padding = 12
                                    border_thickness = 2
                                    ...
                                    # ichki qora fon (o'qishni osonlashtiradi)
                                    cv2.rectangle(frame, (box_x1, box_y1), (box_x2, box_y2), (0, 0, 0), -1)
                                    # tashqi ramka (raqam rangida -- yashil VALID, qizil INVALID)
        ...
        # aniqlangan vagon raqamlari ro'yxati -- yuqori o'ng burchakda,
        # yuqoridan pastga, kichik shrift bilan. RO'YXAT SCROLL BO'LADI:
        # ekranga necha qator sig'sa, aynan shuncha ENG OXIRGI (yangi)
        # raqam ko'rsatiladi -- eskilari yuqoridan avtomatik chiqib ketadi.
        if SHOW_RECOGNIZED_LIST and train_session.wagons:
            ...
            for idx, (number, ts, is_valid) in enumerate(visible_wagons):
                list_color = (0, 255, 0) if is_valid else (0, 0, 255)
                ...
                # o'qishni osonlashtirish uchun ingichka qora "soya"
        ...
        if width > 1920:
            ...
            # YANGI: kichik o'lchamdagi video ekranda juda kichik ko'rinmasligi
            # uchun, faqat KO'RSATISH uchun kattalashtiramiz (saqlanadigan
            # video va aniqlash jarayoni asl o'lchamda davom etadi)
